...
public:
    ...
    int maxProfitDP(int k, vector<int>& prices) {
        // If the stock information is too limited for transaction.
        if (k == 0 || prices.size() < 2) return 0;

        // Avoid unreasonable large k value.
        if (k > prices.size()) {
            int sum = 0;
            for (int i = 1; i < prices.size(); ++i) {
                int diff = prices[i] - prices[i - 1];
        ...
        // buy and sell are initialized on the day 0 case.
        // On day i, there will be at most i transactions. Increasing k has no effect on the final profits.
        ...
        for (int i = 0; i < prices.size(); ++i) {
            for (int j = 0; j < k && j <= i; ++j) {
                sell[j] = max(sell[j] /* Last sell not on day i */, buy[j] + prices[i] /* Last sell on day i */);
                buy[j] = max(buy[j] /*Last buy not no day i*/, (j > 0 ? sell[j - 1] : 0) - prices[i] /*Last buy on day i*/);
            }
            if (i + 1 < k) {
                sell[i + 1] = sell[i];
                buy[i+1] = buy[i];
